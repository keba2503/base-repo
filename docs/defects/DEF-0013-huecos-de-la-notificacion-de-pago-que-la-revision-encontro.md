...
Las dos quedaron nombradas en el pull request para que quien lo revisara decidiera, en vez de fusionarse en silencio. La decisión ya se tomó despierta, y las dos se cierran en un cambio posterior sobre la misma rama.

**Cerrados en el cambio posterior:**

- La lectura que decide la transición ahora ocurre dentro de la misma transacción que la guarda, y esa lectura pide el bloqueo: `PaymentRepository` gana `findByIdForUpdate`, que en `PostgresPaymentRepository` es `findById` con `.for("update")`, de modo que la fila queda bloqueada hasta que la transacción — abierta por `unitOfWork.run` con el tenant del pago, no en ámbito de registro — confirma. `recordProviderPaymentEvent` conserva su primera lectura, sin bloqueo, solo para aprender de qué tenant es el pago antes de poder abrir esa unidad de trabajo; la lectura que de verdad decide la transición es la segunda, la bloqueada, hecha ya dentro de ella. `InMemoryPaymentRepository.findByIdForUpdate` delega en `findById` sin más: un almacén de un solo hilo no tiene fila que bloquear, así que no hay nada que la implementación en memoria deba imitar aparte de la forma del puerto. `packages/application/test/record-provider-payment-event.test.ts` fija que el resultado sigue a la lectura bloqueada y no a la primera, haciendo que el doble de prueba devuelva un estado distinto en cada una; `packages/infrastructure/test/contracts/payment-repository.contract.ts` fija que la lectura bloqueada ve lo mismo que la lectura normal y respeta igual el aislamiento por tenant.
- `ProviderPaymentEvent` gana `tenantId: string | undefined`, que `StripePaymentGateway` rellena desde `metadata.tenantId` del objeto del evento, el mismo sitio de donde ya lee `metadata.paymentId`. `recordProviderPaymentEvent`, una vez resuelto el pago por identificador y antes de aplicar nada, rechaza con `forbidden(paymentNotificationTenantMismatchCode, ...)` cuando el evento trae un tenant que no es el del pago encontrado; un evento sin tenant — el caso de un objeto creado desde el panel de Stripe, sin nuestros metadatos — no se rechaza por eso, y la resolución por identificador sigue gobernando como antes. `packages/infrastructure/test/contracts/payment-gateway.contract.ts` y `packages/infrastructure/test/stripe.test.ts` fijan que el tenant vuelve tal como se envió y que su ausencia dejar el campo indefinido; `packages/application/test/record-provider-payment-event.test.ts` fija el rechazo cuando no coincide y que un evento sin tenant se aplica igual que antes.
