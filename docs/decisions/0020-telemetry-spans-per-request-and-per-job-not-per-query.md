...
`SENTRY_DSN` is optional in every environment, including production: an project derived from this base may not use Sentry, or may not want tracing turned on yet, and forcing it would make this decision's job (choosing what to trace) someone else's forced choice. When it is absent, `container.ts` logs a warning once at boot and falls back to `NoopTelemetry`, never to `InMemoryTelemetry`, which exists only for tests and would otherwise accumulate spans forever in a long-running process. `dispatchJobs` gained a required `telemetry` dependency the same way it already has a required `logger`; every existing caller (application tests, `apps/worker/src/main/use-cases.ts`) was updated to pass one, memory or stub.
